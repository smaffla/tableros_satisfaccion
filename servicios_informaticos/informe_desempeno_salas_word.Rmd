...
desempeno[] <- lapply(desempeno, function(x) gsub("^[a-d]\\)\\s+", "", x))
```
...
En este apartado se muestra la distribución de los administradores de las salas de cómputo en las distintas dependencias de la UPN.

### Tabla
```{r}
desempeno %>% 
  filter(!is.na(dependencia_number_10)) %>% 
  categorica_1var(dependencia_number_10, "Tipo de vinculación")
